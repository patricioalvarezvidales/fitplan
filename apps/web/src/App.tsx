  ...
  const completeSession = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      exercises: activeSession.exercises.map((item) => ({
        ...
        actual_weight_kg: Number(form.get(`weight_${item.id}`) ?? 0),
...
function Brand() { return <button className="brand" onClick={() => location.reload()}><img src="/favicon.svg" alt="" /><span>FITPLAN</span></button> }
...
function HistoryCalendar({ history }: { history: HistoryItem[] }) {
  ...
  })}</div></article><section className="history-list calendar-detail">{selectedItems.length ? selectedItems.map((item) => <article key={item.id}><div><span>{new Date(item.completed_at).toLocaleDateString('es-MX')}</span><h2>{item.session_name}</h2><p>{item.plan_name}</p><details><summary>Ver cargas registradas</summary>{item.exercises.map((exercise) => <p key={exercise.exercise_name}>{exercise.exercise_name}: {exercise.actual_weight_kg} kg · {exercise.completed_sets} series · {exercise.completed_repetitions}</p>)}</details></div><div className="metrics"><b>{item.actual_minutes} min</b><span>Dificultad {item.difficulty}/10</span><span>Energía {item.energy_level}/10</span><span>Satisfacción {item.satisfaction}/10</span>{item.pain_reported && <span className="pain">Molestia: {item.pain_area || 'reportada'}</span>}</div></article>) : <Empty title="Sin entrenamiento ese día" text="Selecciona un día marcado para revisar la sesión y las cargas utilizadas." />}</section></section>
}
